...
标准宗门心法覆盖 1～180 级。目标等级 `L` 的单级研习成本独立于境界突破经验表：
...
- `mine_patrol`：领取时按玩家境界和阶段生成固定的“裂岩獠兽”。五项标准属性按境界预算均分后乘 `0.75`，固定装备三门玄品技能“碎岩扑击”“撼地怒吼”“妖血沸腾”，在本宗灵脉开战。
- `weekly_tournament`：领取时随机锁定同宗、同大境界且双方角色与成员关系均有效的玩家完整战斗构筑；无候选时不创建任务。开战入口位于演武场设施“宗门擂台”。
...
玩家目标快照保存来源角色、来源宗门、锁定时间和完整 `CultivatorCombatInput`；任务视图只返回名称、描述、宗门、境界与阶段摘要，不暴露技能和装备数据。小比候选只按 `realm` 匹配同大境界；讨伐悬赏候选匹配同大境界或低一大境界，阶段均可以不同。玩家后续换装、突破、转宗、死亡或注销不会改变已领取任务中的目标。
