...
export function readSpiritSeedDetails(value: unknown): SpiritSeedDetails | null {
...
export const CULTIVATION_METHODS: readonly CultivationMethodDefinition[] = [
  { id: 'slow_nurture', name: '顺时温养', description: '循草木时序温养，不额外耗费资源。', stages: ['germination', 'growth'], methodTags: ['slow_nurture'], environmentTags: ['balanced_soil'], minGardenLevel: 1, cost: { kind: 'time' } },
  { id: 'qi_acceleration', name: '天地灵气催生', description: '引天地灵气催动生机，能缩短本阶段时长。', stages: ['germination', 'growth'], methodTags: ['qi_acceleration'], environmentTags: ['qi_dense'], minGardenLevel: 1, cost: { kind: 'qi', amount: 20 } },
  { id: 'spirit_stone_stabilization', name: '灵石固壤', description: '以灵石稳住土中灵机，使萌芽少生偏差。', stages: ['germination'], methodTags: ['spirit_stone_stabilization'], environmentTags: ['qi_dense'], minGardenLevel: 1, cost: { kind: 'spirit_stones', amount: 100 } },
  { id: 'sunlit_awakening', name: '向阳醒种', description: '借日照与干暖土性唤醒种中生机。', stages: ['germination'], methodTags: ['slow_nurture'], environmentTags: ['sunlit_dry'], minGardenLevel: 2, cost: { kind: 'time' } },
  { id: 'shade_dew', name: '荫棚集露', description: '遮去烈日，以清凉水汽润开籽壳。', stages: ['germination'], methodTags: ['slow_nurture'], environmentTags: ['shaded_cool', 'moist_watered'], minGardenLevel: 2, cost: { kind: 'time' } },
  { id: 'ore_soil', name: '矿砂改土', description: '投入一份矿石，以其材性改变根系所依土壤。', stages: ['germination'], methodTags: ['ore_soil'], environmentTags: ['mineral_rich'], minGardenLevel: 2, cost: { kind: 'material', materialType: 'ore', amount: 1 } },
  { id: 'root_resonance', name: '本命灵力灌注', description: '选择自身灵根，消耗法力与灵植共鸣。', stages: ['growth'], methodTags: ['root_resonance'], environmentTags: ['qi_dense'], minGardenLevel: 3, cost: { kind: 'mp', amount: 20 }, requiresRoot: true },
  { id: 'herb_companion', name: '药材伴养', description: '投入一份药材，引导幼株吸纳相近药性。', stages: ['growth'], methodTags: ['herb_companion'], environmentTags: ['balanced_soil'], minGardenLevel: 3, cost: { kind: 'material', materialType: 'herb', amount: 1 } },
  { id: 'monster_blood', name: '妖血沃根', description: '投入一份妖兽材料，激发旺盛而不稳定的生机。', stages: ['growth'], methodTags: ['monster_blood'], environmentTags: ['balanced_soil'], minGardenLevel: 3, cost: { kind: 'material', materialType: 'monster', amount: 1 } },
  { id: 'aux_formation', name: '辅材布阵', description: '以辅材和灵石在畦旁布下聚灵小阵。', stages: ['germination', 'growth'], methodTags: ['aux_formation'], environmentTags: ['qi_dense'], minGardenLevel: 4, cost: { kind: 'material', materialType: 'aux', amount: 1, spiritStones: 50 } },
] as const;
...
export const FORMATION_METHODS: readonly FormationMethodDefinition[] = [
  { id: 'leaf_medicine', name: '凝叶成药', description: '引导药性归入枝叶，成型稳定，产量较高。', outcomeBias: 'herb' },
  { id: 'fruit_bloom', name: '开花结果', description: '引导灵机结为可服用灵果，成败取决于此前长势。', outcomeBias: 'spirit_fruit' },
  { id: 'treasure_return', name: '返源化宝', description: '尝试将草木精华凝成天材地宝，对品质和长势要求最高。', outcomeBias: 'tcdb' },
  { id: 'natural_form', name: '顺势化形', description: '不强定形态，由种子隐藏天性自行成型。' },
] as const;
...
export function findCultivationMethod(id: string): CultivationMethodDefinition | undefined {
  return CULTIVATION_METHODS.find((method) => method.id === id);
}

export function findFormationMethod(id: string): FormationMethodDefinition | undefined {
...
function fallbackStageCopy(assessment: StageAssessment) {
  switch (assessment) {
    case 'resonant': return { hint: '幼叶舒展，灵纹随所施之法缓缓亮起。', narrative: '草木灵机与此法彼此呼应，根叶间生出清润光泽。' };
    case 'aligned': return { hint: '根须安稳扎入土中，气息渐趋匀净。', narrative: '这轮照料顺应了灵植长势，虽无异象，生机却更显稳固。' };
    case 'conflict': return { hint: '根须短暂蜷缩，叶缘也失了几分润色。', narrative: '所施之法与种性相冲，灵机虽未断绝，运转却明显滞涩。' };
    case 'neutral': return { hint: '枝叶未显偏转，只按原有时序缓慢生长。', narrative: '灵植平稳承受了本轮培育，暂时看不出明显喜恶。' };
  }
}
...
export function resolveCultivationMethod(
  ...
  if (input.materialElement && input.seedElement && input.materialElement === input.seedElement)
    ruleScore += 1;
  const allowedAssessments: StageAssessment[] = ruleScore >= 3
    ? ['resonant', 'aligned']
    : ruleScore >= 0
      ? ['aligned', 'neutral']
      : ['neutral', 'conflict'];
  const fallbackAssessment = ruleScore >= 3 ? 'resonant' : ruleScore >= 0 ? 'aligned' : 'conflict';
  const fallback = fallbackStageCopy(fallbackAssessment);
  return {
    ruleScore,
    scoreDelta: Math.max(-12, Math.min(24, ruleScore * 6 + 6)),
    durationMultiplier: methodId === 'qi_acceleration' ? 0.7 : ruleScore < 0 ? 1.12 : ruleScore >= 3 ? 0.9 : 1,
    allowedAssessments,
    fallbackAssessment,
...
export function resolveOutcomeKind(
  ...
  if (formationMethodId === 'fruit_bloom') {
    const chance = Math.max(0.35, Math.min(0.9, 0.52 + score / 240 + (spec.outcomeBiases.includes('spirit_fruit') ? 0.15 : 0)));
    return roll < chance ? 'spirit_fruit' : 'herb';
  }
  if (formationMethodId === 'treasure_return') {
    const qualityIndex = QUALITY_ORDER.indexOf(seedRank);
    const chance = Math.max(0.05, Math.min(0.72, 0.08 + qualityIndex * 0.06 + score / 260 + (spec.outcomeBiases.includes('tcdb') ? 0.16 : 0)));
    return qualityIndex >= 2 && roll < chance ? 'tcdb' : 'herb';
  }
  if (spec.outcomeBiases.includes('tcdb') && roll < Math.max(0.04, Math.min(0.25, score / 400))) return 'tcdb';
  if (spec.outcomeBiases.includes('spirit_fruit') && roll < Math.max(0.22, Math.min(0.65, 0.28 + score / 260))) return 'spirit_fruit';
  return 'herb';
}

export function resolveOutcomeQuality(seedRank: Quality, score: number, roll: number): Quality {
  const index = QUALITY_ORDER.indexOf(seedRank);
  const riseChance = Math.max(0, Math.min(0.28, (score - 24) / 220));
  const fallChance = Math.max(0, Math.min(0.24, (18 - score) / 120));
  if (roll < riseChance) return QUALITY_ORDER[Math.min(QUALITY_ORDER.length - 1, index + 1)];
  if (roll > 1 - fallChance) return QUALITY_ORDER[Math.max(0, index - 1)];
  return seedRank;
}
...
export type HerbGardenPlotStatus = 'empty' | 'cultivating' | 'awaiting_action' | 'ready';
...
export interface HerbGardenPlotView {
  ...
  outcomePreview?: { name: string; kind: HerbGardenOutcomeKind; rank: Quality };
...
export interface HerbGardenState {
  ...
  methodMaterials: Array<{ materialId: string; name: string; type: Exclude<MaterialType, 'seed'>; rank: Quality; element?: ElementType; quantity: number }>;
  spiritualRoots: Array<{ element: ElementType; strength: number }>;
  logs: HerbGardenLogView[];
  friends: HerbGardenFriendView[];
  summary: { planted: number; awaitingAction: number; ready: number; totalHarvests: number };
}
