  ...
  return (
    <div className="space-y-6">
      <header className="border-ink/15 bg-bgpaper/90 border border-dashed p-6">
        ...
        <p className="text-ink-secondary mt-3 text-sm">支持角色名、邮箱、角色 ID、用户 ID 检索，查看资产与进度后通过游戏邮件补发。</p>
      </header>
      <section className="border-ink/15 bg-bgpaper/90 border border-dashed p-5">
        <div className="flex gap-2"><input className="border-ink/20 min-w-80 border px-3 py-2" value={keyword} onChange={(event) => setKeyword(event.target.value)} placeholder="角色名 / 邮箱 / ID" /><button onClick={() => void search()}>检索</button></div>
        <div className="mt-4 grid gap-2 md:grid-cols-2">{players.map((player) => <button key={player.cultivatorId} className="border-ink/15 border p-3 text-left" onClick={() => void select(player.cultivatorId)}><strong>{player.name}</strong> · {player.realm}{player.stage}<br /><span className="text-xs">{player.email} · {player.cultivatorId}</span></button>)}</div>
      </section>
      {detail ? <section className="border-ink/15 bg-bgpaper/90 space-y-5 border border-dashed p-5">
        <h3 className="font-heading text-2xl">{detail.player.name} · {detail.player.realm}{detail.player.stage}</h3>
        <div className="grid gap-3 text-sm md:grid-cols-4"><div>灵石 {detail.player.spiritStones}</div><div>声望 {detail.player.reputation}</div><div>材料 {detail.inventory.materialQuantity}</div><div>未领邮件 {detail.progress.unclaimedRewardMails}</div><div>任务 {detail.progress.activeTasks}/{detail.progress.completedTasks}</div><div>副本记录 {detail.progress.dungeonRuns}</div><div>宗门 {detail.sect?.sectId ?? '无'}</div><div>最后活跃 {detail.player.lastActiveAt ? new Date(detail.player.lastActiveAt).toLocaleString() : '暂无'}</div></div>
