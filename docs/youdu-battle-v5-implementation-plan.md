...
>
> 本文是 2026-07-22 旧 `BattleEngineV5` 1v1 基线上的历史实现分析，用于追溯幽都通用原语与内容层取舍，不再定义当前战斗主流程。旧引擎及其 1v1 限制已经删除；当前生产基线为 `Team/Roster + BattleRuntime + Checkpoint + BattleRoundResolver`。涉及多目标、行动、胜负、回放与运行时接口时必须以当前代码和 `docs/sect-authoring-guide.md` 为准。
>
...
- 当时的 V1 删除项：三目标忘川、向其他敌人扩散蚀魂、百鬼同哭影响全体。当时生产主流程严格 1v1，因此没有为单个宗门提前扩建队伍战斗；这条历史降级理由不再代表当前 Team/Roster 引擎能力。
...
| 领域 | 当前事实 | 对幽都的影响 |
| --- | --- | --- |
| 战斗人数（历史基线） | 当时的 `BattleEngineV5` 只持有 `player` 与 `opponent`，目标选择系统未接入主行动流 | 此限制已随旧引擎删除；当前多单位能力必须按 Team/Roster 代码重新验收 |
...
- 当时 `BattleEngineV5` 的 player/opponent 模型（已删除）
- 行动排序和默认目标
...
- 三目标忘川与多敌扩散：当时的 1v1 架构没有生产语义；当前 Team/Roster 架构下需要重新立项验证。
- 双幽都共享蚀魂：当时生产战斗没有同队多单位；当前仍缺少明确玩法规则，不能直接视为已支持。
- 动画、音效和受击表现：设计稿已明确暂缓。
